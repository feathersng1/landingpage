...
export function NotrifyMainMedia() {
  return (
    ...
    >
      <div className="max-w-[1440px] mx-auto flex flex-col gap-6 md:gap-12 px-0 md:px-6">
        
        {/* Row 1: App Home Preview (Responsive) */}
        <motion.div
          variants={itemVariants}
          className="relative w-full min-h-[400px] md:min-h-[600px] lg:min-h-[800px] bg-[#BCC0E1] overflow-hidden flex flex-col items-center justify-center py-10 md:py-16 lg:py-24 rounded-none md:rounded-[16px]"
        >
          {/* Background Image - Focus on right to reveal ribbon on mobile */}
          <img 
          ...
          {/* App Preview - Carefully shifted to balance with ribbon */}
          <div className="relative z-10 w-full h-full flex items-center justify-center -translate-x-[10%] md:-translate-x-[12%] lg:-translate-x-[10%] px-6">
            <picture>
              ...
              <img 
                ...
                className="max-w-full max-h-full h-auto object-contain drop-shadow-2xl"
              />
            </picture>
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 px-6 md:px-0">
          {/* Card 1: Profile/Onboarding Form (Width fill, consistent padding) */}
          <motion.div
            variants={itemVariants}
            className="w-full bg-[#F5F5F5] rounded-[16px] aspect-square flex flex-col p-4 md:p-8 overflow-hidden group"
          >
            <div className="flex-1 flex items-center justify-center overflow-hidden w-full">
              <motion.img 
                src="/notrify/profile.svg" 
                alt="Profile Setup" 
                className="w-full h-auto object-contain rounded-[8px] md:rounded-[12px] transform transition-transform duration-700 group-hover:scale-105"
              />
            </div>
          </motion.div>
